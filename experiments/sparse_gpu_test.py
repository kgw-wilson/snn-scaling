...
def run_simulation_sparse_gpu(graph_config: ERGraphConfig, snn_config: SNNConfig):
    ...
    binary_spikes = torch.zeros(N, device=graph_config.device, dtype=torch.bool)
    synaptic_currents = torch.zeros(
        N, device=graph_config.device, dtype=graph_config.dtype
    )
    ...
    with MonitoringWindow("simulation main loop"):
        ...
        for _ in range(snn_config.num_timesteps):
            ...
            spike_vector = binary_spikes.to(graph_config.dtype).unsqueeze(1)

            synaptic_input = torch.sparse.mm(weights, spike_vector).squeeze(1)
...
if __name__ == "__main__":
    ...
    run_simulation_sparse_gpu(graph_config, snn_config)
