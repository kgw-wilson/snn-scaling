...
def clock_driven_sparse_gpu(
    ...
    bucketized_weights, bucket_offsets = build_sparse_weights_bucketized_by_delay(
        graph_config, snn_config, use_numpy=False
    )
...
def _simulation_step(
    ...
    Similar to the dense version, except uses sparse weights.

    TODO: speedup when using compiled?
    """
    ...
    # Will this work? Previously:
    # for bucket_idx in range(len(bucketized_weights)):
    #     bucket_idx_in_buffer = (
    #         buffer_idx + bucket_0_offset + bucket_idx
    #     ) % buffer_size
    #     ring_buffer[bucket_idx_in_buffer] = (
    #         bucketized_weights[bucket_idx] @ spikes
    #     )
